...
def normalize_episode_timestamps(episode_path: Path, fps: int = 30) -> int:
    ...
    1. Converts nanosecond timestamps to absolute seconds.
    2. Converts them to relative seconds starting at 0.
    3. Resamples timestamps so they are uniformly spaced at 1/fps seconds.
    ...
    dt: float = 1.0 / fps  # 30 FPS → 0.033333...
    ...
    with csv_path.open("r", newline="") as fin, temp_path.open("w", newline="") as fout:
        ...
        # Convert first timestamp from ns → seconds
        first_timestamp_sec: float = first_timestamp_ns / 1e9

        # First row always becomes 0.0 seconds
        first_data_row[0] = f"{0.0:.9f}"
        ...
        for row in reader:
            try:
                ...
                current_timestamp_sec = current_timestamp_ns / 1e9

                # Relative seconds (computed but not used for spacing —
                # we enforce strict 30 FPS uniform timestamps)
                _relative_sec = current_timestamp_sec - first_timestamp_sec

                # Enforce uniform 30 FPS timestamps
